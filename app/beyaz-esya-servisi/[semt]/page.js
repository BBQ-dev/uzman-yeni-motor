...
export default function Page({ params }) {const { semt: rawSemt } = React.use(params);
  ...
  if (cihazKuyrugu === 'buzdolabi') {
    ...
    if (arizaKuyrugu === 'arcelik') anaTabela = `${location} ARÇELİK BUZDOLABI SERVİSİ`;
    if (arizaKuyrugu === 'beko') anaTabela = `${location} BEKO BUZDOLABI SERVİSİ `;
    if (arizaKuyrugu === 'altus') anaTabela = `${location} ALTUS BUZDOLABI SERVİSİ`;
    if (arizaKuyrugu === 'bosch') anaTabela = `${location} BOSCH BUZDOLABI SERVİSİ`;
    if (arizaKuyrugu === 'siemens') anaTabela = `${location} SİEMENS BUZDOLABI SERVİSİ`;
    if (arizaKuyrugu === 'vestel') anaTabela = `${location} VESTEL BUZDOLABI SERVİSİ`;
    if (arizaKuyrugu === 'profilo') anaTabela = `${location} PROFİLO BUZDOLABI SERVİSİ`;
    if (arizaKuyrugu === 'buzdolabitamircisi') anaTabela = `${location} BUZDOLABI TAMİRCİSİ `;
    if (arizaKuyrugu === 'buzdolabitamiri') anaTabela = `${location} BUZDOLABI TAMİRİ`;
    if (arizaKuyrugu === 'buzdolabisogutmuyor') anaTabela = `${location} Buzdolabı Alt Kısım Soğutmuyor İse Hemen Ulaşın`;
    if (arizaKuyrugu === 'buzdolabitamircileri') anaTabela = `${location} BUZDOLABI TAMİRCİLERİ'NİN EN İYİSİ`;
  }
...
if (cihazKuyrugu === 'dondurucu') {
    ...
    if (arizaKuyrugu === 'ugur') anaTabela = `${location} UĞUR DERİN DONDURUCU SERVİSİ`;
    if (arizaKuyrugu === 'bosch') anaTabela = `${location} BOSCH DERİN DONDURUCU SERVİSİ`;
    if (arizaKuyrugu === 'arcelik') anaTabela = `${location} ARÇELİKDERİN DONDURUCU SERVİSİ`;
    if (arizaKuyrugu === 'beko') anaTabela = `${location} BEKO DERİN DONDURUCU SERVİSİ`;
    if (arizaKuyrugu === 'vestel') anaTabela = `${location} VESTEL DERİN DONDURUCU SERVİSİ`;
    if (arizaKuyrugu === 'altus') anaTabela = `${location} ALTUS DERİN DONDURUCU SERVİSİ`;
    if (arizaKuyrugu === 'siemens') anaTabela = `${location} SİEMENS DERİN DONDURUCU SERVİSİ`;
    if (arizaKuyrugu === 'profilo') anaTabela = `${location} PROFİLO DERİN DONDURUCU SERVİSİ`;
    if (arizaKuyrugu === 'indesit') anaTabela = `${location} İNDESİT DERİN DONDURUCU SERVİSİ`;
}
